 ...
 *
 * The sequence does not begin until all three scene images have loaded and decoded.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // These are intentional pauses, not animation durations. CSS controls fade/drop speed.
   ...
   * decode() avoids beginning a fade only to stall on the first frame that needs the image.
   */
  const prepareImage = async (image) => {
    ...
    if (typeof image.decode === 'function') {
      try {
        ...
        // Some browsers can reject decode() even though the image is already renderable.
        // naturalWidth above is the final sanity check, so this is safe to continue from.
   ...
   * transitionend is not emitted if a transition is cancelled or has zero duration.
   */
  ...
  const finishImmediately = () => {
    // Prefer crown.webp, then bt-2, then bt-1 if an asset failed. This prevents a
  ...
  const runOpeningSequence = async () => {
    try {
      // Wait for only the images this sequence actually needs. Using window.load would
